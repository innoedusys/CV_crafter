...
const ROLE_SUGGESTIONS = [
  ...
  'Product Manager', 'Mobile Developer', 'Cloud Engineer', 'Marketing Manager',
  'Project Manager', 'Graphic Designer', 'Cybersecurity Engineer', 'Student',
];
    ...
    <div className="space-y-6 overflow-y-auto p-6">
      ...
      <div className="rounded-lg bg-gradient-to-r from-primary/10 to-primary/5 border border-primary/20 p-4">
        <div className="flex items-start gap-3">
          ...
          <div>
            <p className="text-xs font-semibold text-foreground">Smart Auto-Fill ✨</p>
            <p className="text-xs text-muted-foreground mt-0.5">
              Just enter your <strong>Name</strong> and <strong>Role</strong> — AI will generate your full CV automatically.
            </p>
          </div>
        </div>
      </div>
      ...
      <Section icon={User} title="Personal Information">
        <div className="grid grid-cols-2 gap-3">
          ...
          <input className={inputClass} placeholder="Email (optional)" value={resumeData.personalInfo.email} onChange={(e) => updatePersonalInfo('email', e.target.value)} />
          <input className={inputClass} placeholder="Phone (optional)" value={resumeData.personalInfo.phone} onChange={(e) => updatePersonalInfo('phone', e.target.value)} />
          <input className={inputClass} placeholder="Location (optional)" value={resumeData.personalInfo.location} onChange={(e) => updatePersonalInfo('location', e.target.value)} />
          <input className={inputClass} placeholder="Website (optional)" value={resumeData.personalInfo.website} onChange={(e) => updatePersonalInfo('website', e.target.value)} />
        </div>
      </Section>
      ...
      <Section icon={Type} title="Professional Summary">
        <textarea
          ...
          placeholder="Write a brief professional summary... (auto-generated from your role)"
            ...
            {resumeData.education.map((edu) => (
              <Reorder.Item key={edu.id} value={edu}>
                <div className="relative space-y-3 rounded-lg border bg-muted/30 p-4">
                  ...
                  <div className="grid grid-cols-2 gap-3 pl-5">
                    <input className={inputClass} placeholder="School / University" value={edu.school} onChange={(e) => updateEducation(edu.id, 'school', e.target.value)} />
                    <input className={inputClass} placeholder="Degree (e.g. B.Sc.)" value={edu.degree} onChange={(e) => updateEducation(edu.id, 'degree', e.target.value)} />
                    ...
                    <input className={inputClass} placeholder="Start Year" value={edu.startDate} onChange={(e) => updateEducation(edu.id, 'startDate', e.target.value)} />
                    <input className={inputClass} placeholder="End Year (or Present)" value={edu.endDate} onChange={(e) => updateEducation(edu.id, 'endDate', e.target.value)} />
                  </div>
                </div>
              </Reorder.Item>
            ))}
